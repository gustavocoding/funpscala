...
class Chapter5Spec extends FlatSpec {
  ...
  "Exercise 5.11" should "generate streams using unfold" in {
    val naturalsLt20 = Stream.unfold(1)(s => if (s < 20) Some((s, s + 1)) else None)

    naturalsLt20.take(30).toList.size shouldBe 19
  }

  "Exercise 5.12" should "do fibs,from,constant,and ones in terms of unfold" in {
    Stream.fromU(2).take(5).toList shouldBe List(2, 3, 4, 5, 6)
    Stream.constantU("A").take(2).toList shouldBe List("A", "A")
    Stream.onesU.take(3).toList shouldBe List(1, 1, 1)
    Stream.fibsU().take(1).toList shouldBe List(0)
    Stream.fibsU().take(2).toList shouldBe List(0, 1)
    Stream.fibsU().take(3).toList shouldBe List(0, 1, 1)
    Stream.fibsU().take(8).toList shouldBe List(0, 1, 1, 2, 3, 5, 8, 13)
  }
}
